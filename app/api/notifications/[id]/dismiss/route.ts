...
export async function POST(_request: Request, context: Context) { const { id } = await context.params; const notification = notificationManager.get(id, familyId); if (!notification) return NextResponse.json({ error: "Notification not found" }, { status: 404 }); return NextResponse.json({ notification: notificationManager.dismiss(id, familyId, notification.memberId) }); }
